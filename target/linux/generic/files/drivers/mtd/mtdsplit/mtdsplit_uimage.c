...
#define MOTO_PAD_LEN		96

static ssize_t uimage_find_moto(u_char *buf, size_t len, int *extralen)
{
	if (uimage_verify_default(buf, len, extralen) < 0)
		return -EINVAL;

	*extralen = MOTO_PAD_LEN;

	return 0;
}

static int
mtdsplit_uimage_parse_moto(struct mtd_info *master,
			      struct mtd_partition **pparts,
			      struct mtd_part_parser_data *data)
{
	return __mtdsplit_parse_uimage(master, pparts, data,
...
static struct mtd_part_parser uimage_moto_parser = {
	.owner = THIS_MODULE,
	.name = "moto-fw",
	.parse_fn = mtdsplit_uimage_parse_moto,
	.type = MTD_PARSER_TYPE_FIRMWARE,
};
...
static int __init mtdsplit_uimage_init(void)
{
	...
	register_mtd_parser(&uimage_moto_parser);

...
#define flash_read(_ad, _ptr, _offset, _len) ra_mtd_read(MTD_NUM_FAC
#define flash_write(_ptr, _offset, _len) ra_mtd_write(MTD_NUM_FACTORY, 0, (size_t)_len, _ptr)

#else
extern int mtk_mtd_write_nm(char *name, loff_t to, size_t len, const u_char *buf);
extern int mtk_mtd_read_nm(char *name, loff_t from, size_t len, u_char *buf);
#define flash_read(_ad, _ptr, _offset, _len) mtk_mtd_read_nm("Factory", _offset&0xFFFF, (size_t)_len, _ptr)
#define flash_write(_ptr, _offset, _len) mtk_mtd_write_nm("Factory", _offset&0xFFFF, (size_t)_len, _ptr)

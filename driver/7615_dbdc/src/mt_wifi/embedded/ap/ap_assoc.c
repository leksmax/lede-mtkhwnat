...
/* WPS_BandSteering Support */
	...
	for (i = 0; i < ie_list->SupportedRatesLen; i++) {
		...
		if (IS_MAP_ENABLE(pAd)) {
			...
			if (Elem->MsgLen > ASSOC_REQ_LEN_R2)
				pEntry->assoc_req_len_R2 = ASSOC_REQ_LEN_R2;
			else
				pEntry->assoc_req_len_R2 = Elem->MsgLen - sizeof(HEADER_802_11);
			NdisMoveMemory(pEntry->assoc_req_frame_R2,
				&Elem->Msg[sizeof(HEADER_802_11)],
				pEntry->assoc_req_len_R2);
		}
#endif
#endif

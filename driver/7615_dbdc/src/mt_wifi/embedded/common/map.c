...
#ifdef MAP_R2
	if (RTMPGetKeyParameter("MapR2Enable", tmpbuf, 25, pBuffer, TRUE)) {
		pAd->bMapR2Enable = (UCHAR) os_str_tol(tmpbuf, 0, 10);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("MapR2Enable=%d\n", pAd->bMapR2Enable));
	}
#endif
...
static inline BOOLEAN get_vlanid_from_pkt(PNDIS_PACKET pkt, UINT16 *pvlanid, UCHAR *pPCP)
{
	...
	if (pPCP)
		*pPCP = ((OS_NTOHS(veth->h_vlan_TCI) & 0xE000) >> 13);
...
BOOLEAN add_vlan_tag(void *packet, UINT16 vlan_tci)
{
...
BOOLEAN is_vid_configed(UINT16 vid, UCHAR vid_num, UINT16 vids[])
{
	UCHAR i = 0;
...
static inline BOOLEAN is_configed_policy(struct wifi_dev *wdev)
{
	return (wdev->MAPCfg.vid_num != 0) ? 1 : 0;
}
...
{
	UINT16 pkt_vid = 0, vlan_tci = 0, conf_vid = 0;
	...
	if (!IS_MAP_R2_ENABLE(pAd) || !IS_VALID_VID(conf_vid))
		goto suc;

	vlan_tagged = get_vlanid_from_pkt(pkt, &pkt_vid, &pcp);
	pSrcBuf = GET_OS_PKT_DATAPTR(pkt);

	/*pure fronthaul bss*/
	if ((wdev->MAPCfg.DevOwnRole & BIT(MAP_ROLE_FRONTHAUL_BSS)) &&
		!(wdev->MAPCfg.DevOwnRole & BIT(MAP_ROLE_BACKHAUL_BSS))) {
		if (!is_configed_policy(wdev))
			goto suc;

		if (vlan_tagged) {
			/*remove the vlan tag if vid matched, else drop it*/
			if (pkt_vid == conf_vid) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("%s() %s, to STA, remove matched vid=%d, pcp=%d.\n",
					__func__, wdev->if_dev->name, pkt_vid, pcp));
				remove_vlan_tag(pAd, pkt);
			} else {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("%s() %s, to STA, drop dismatched pkt vid=%d.\n",
					__func__, wdev->if_dev->name, pkt_vid));
				goto fail;
			}
		}
	...
	/*backhaul bss*/
	if (wdev->MAPCfg.DevOwnRole &
		(BIT(MAP_ROLE_BACKHAUL_BSS) | BIT(MAP_ROLE_BACKHAUL_STA))) {
		if (vlan_tagged) {
			if (!is_configed_policy(wdev))
				goto suc;
			...
			if (peer_entry->DevPeerRole == 0) {
				...
				if (pkt_vid == conf_vid) {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						("%s() %s, to R1, remove matched vid=%d, pcp=%d.\n",
						__func__, wdev->if_dev->name, pkt_vid, pcp));
					remove_vlan_tag(pAd, pkt);
				} else
					goto fail;
			...
			if (peer_entry->profile == 0x02) {
				...
				*/
				...
				if (IS_VALID_VID(wdev->MAPCfg.primary_vid) &&
					...
					pcp = wdev->MAPCfg.primary_pcp;
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						("%s() %s, to R2, add tag with vid=%d, pcp=%d.\n",
						__func__, wdev->if_dev->name, wdev->MAPCfg.primary_vid,	pcp));
					/*add primary vlan id for 1905 message*/
					vlan_tci |= 0x0fff & wdev->MAPCfg.primary_vid;
					vlan_tci |= (pcp & 0x07) << 13;
					if (!add_vlan_tag(pkt, vlan_tci))
						goto fail;
				}
			}
		}
...
BOOLEAN map_ts_rx_process(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR wdev_idx)
{
	UINT16 pkt_vid = 0, vlan_tci = 0, conf_vid = 0;
	BOOLEAN vlan_tagged = FALSE;
	UCHAR *pSrcBuf = NULL;
	UCHAR up = 0xFF, dscp = 0xFF, pcp = 0;
	FRAME_CONTROL *pFmeCtrl;
	struct wifi_dev *wdev;
	MAC_TABLE_ENTRY *peer_entry;
	struct sk_buff *pkt;
	UINT16 pkt_type = 0;

	wdev = pAd->wdev_list[wdev_idx];
	peer_entry = &pAd->MacTab.Content[pRxBlk->wcid];
	if (!wdev || !peer_entry)
		goto suc;

	if (!IS_MAP_R2_ENABLE(pAd) || (!is_configed_policy(wdev)))
		goto suc;

	conf_vid = map_get_dev_vid(wdev);
	pkt = RTPKT_TO_OSPKT(pRxBlk->pRxPacket);
	vlan_tagged = get_vlanid_from_pkt(pkt, &pkt_vid, NULL);

	if (vlan_tagged == FALSE) {
		...
		if (pkt_type == ETH_TYPE_IPv4)
			dscp = ((pSrcBuf[15] & 0xfc) >> 2);
		else if (pkt_type == ETH_TYPE_IPv6)
			dscp = (((pSrcBuf[14] & 0x0f) << 2) | (pSrcBuf[15] & 0xc0) >> 6);
		else
			dscp = 0xFF;
	}

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s() on %s, DevPeerRole=%02x, profile=%02x, up=%d, dscp=%d\n",
		__func__, wdev->if_dev->name, peer_entry->DevPeerRole, peer_entry->profile, up, dscp));

	/*pure fronthaul bss*/
	if ((wdev->MAPCfg.DevOwnRole & BIT(MAP_ROLE_FRONTHAUL_BSS)) &&
		!(wdev->MAPCfg.DevOwnRole & BIT(MAP_ROLE_BACKHAUL_BSS))) {
		if (vlan_tagged) {
			/*drop the packet with vlan which is received from pure front haul bss*/
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("%s() %s, from STA, drop ptk with vid=%d.\n",
					__func__, wdev->if_dev->name, pkt_vid));
			goto fail;
		} else {
			/*if this bss has been configured, add the corresponding vid,
			* or else add primary vid.
			*/
			if (conf_vid) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("%s() %s, from STA, add vid=%d\n",
					__func__, wdev->if_dev->name, conf_vid));

				pcp = wdev->MAPCfg.primary_pcp;

				vlan_tci |= 0x0fff & conf_vid;
				vlan_tci |= (pcp & 0x07) << 13;
				if (!add_vlan_tag(pkt, vlan_tci))
					goto fail;
			}
	...
	if (wdev->MAPCfg.DevOwnRole &
		...
		if (vlan_tagged) {
			if (peer_entry->DevPeerRole == 0 ||
				...
				/*	normal sta
				 *	drop the packet with vlan which is send from
				 *	normal station or map r1 device
				 */
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
					("%s() %s, from R1/STA, drop ptk with vid=%d\n",
					__func__, wdev->if_dev->name, pkt_vid));
				goto fail;
			} else if (peer_entry->profile == 0x02) {
				/*	map r2 device check whether the vid is included
				 ...
				 */
				if (!is_vid_configed(pkt_vid, wdev->MAPCfg.vid_num,
					wdev->MAPCfg.vids) &&
					wdev->MAPCfg.primary_vid != pkt_vid) {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						("%s() %s, from R2,"
						"drop ptk with vid =%d that is not in ts policy\n",
						__func__, wdev->if_dev->name, pkt_vid));
					goto fail;
				}
			}
			...
			if (peer_entry->DevPeerRole == 0) {
				...
				*/
				if (conf_vid) {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
						("%s() %s, from STA, add for station vid=%d\n",
						__func__, wdev->if_dev->name, conf_vid));
				...
				if (IS_VALID_VID(wdev->MAPCfg.primary_vid)) {
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN,
							("%s() %s, from R1, add vid=%d\n",
							__func__, wdev->if_dev->name, wdev->MAPCfg.primary_vid));

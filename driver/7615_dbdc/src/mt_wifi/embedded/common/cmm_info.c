...
INT Set_Map_Bh_Clear_Vlan_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	POS_COOKIE	pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR if_idx = pObj->ioctl_if;
	struct wifi_dev *wdev = get_wdev_by_ioctl_idx_and_iftype(pAd, if_idx, pObj->ioctl_if_type);
...
INT Set_Map_Bh_Vid_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	wdev->MAPCfg.vids[wdev->MAPCfg.vid_num++] = vid;
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: %s add vid=%d, vid_num=%d\n", __func__, wdev->if_dev->name, vid, wdev->MAPCfg.vid_num));
